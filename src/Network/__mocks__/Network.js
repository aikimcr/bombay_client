...
export function getFromURLString(url, query) {
  throw new Error(
    `getFromURLString has not been mocked properly ${url} ${JSON.stringify(query)}`,
  );
}

export async function postToURLString(urlString, body) {
  throw new Error(
    `postToURLString has not been mocked properly ${url} ${JSON.stringify(body)}`,
  );
}

export async function putToURLString(urlString, body) {
  throw new Error(
    `putToURLString has not been mocked properly ${url} ${JSON.stringify(body)}`,
  );
}
